Config {
    ...
    commands = [
        ...
        Run Wireless "wlp1s0" [ "-t", "<fn=1>\xf1eb</fn> <fc=#ffca28><essid></fc> [<qualitybar>]","-L","0","-H","32","--normal","yellow","--high","#ffca28","--low","red"] 10,
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{%multicpu% %memory% %swap%  %wlp1s0wi% <fn=1></fn> <fc=#ffca28>%volume%</fc> %battery% %date% %EGLL%"
}
